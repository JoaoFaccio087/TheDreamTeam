...
## UX online — sobreposição de nomes no campo (CORRIGIDO — 1/4)
Nos prints do João, etiquetas de nome (Rodri/Dunga/Hakimi) se sobrepunham às fichas em
formações compactas (4-1-2-1-2, 3-4-3 etc.). Causa: `.slot-nome` fica em `top:100%`
(embaixo) e invadia a ficha logo abaixo.
- Solução (sem mexer nas coordenadas das formações — receio do João de "desvirtuar"):
  etiqueta ADAPTATIVA. Em `js/ui.js`, `posicionarCampo` detecta se há ficha logo abaixo
  (dТop 8–20%, dLeft <28%) e adiciona classe `nome-acima`; CSS (`css/online.css`) então
  joga o nome para cima da bolinha (`bottom:100%`). +z-index p/ etiqueta nunca sumir atrás.
  Vale p/ TODAS as formações e p/ todos os campos (online/solo/draft compartilham a função).
- Arquivos: `js/ui.js`, `css/online.css`. Deploy: GitHub Pages.
- PENDENTE (próximos 3 ajustes de UX online pedidos pelo João):
  (2) trazer tabelas de estatística p/ a aba Mata-a-Mata (como no modo Copa solo);
  (3) aba Partidas sem botão "próxima"/avançar após fase encerrada;
  (4) botões de fim de jogo (Resumo/Voltar) só aparecem na aba Grupos, lá embaixo —
      deveriam estar acessíveis em todas as abas.
